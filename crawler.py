...
class WebCrawler:
    ...
    def download_file(self, url: str) -> bool:
        ...
        if not response or response.status_code != 200:
            ...
            if self.status_callback:
                self.status_callback('download_failed', {'url': url, 'reason': 'Failed to retrieve'})
            return False
        ...
        if not file_type_detected:
            ...
            if self.status_callback:
                self.status_callback('download_failed', {'url': url, 'reason': 'Unknown file type'})
            return False
        ...
        if self.status_callback:
            self.status_callback('download_started', {'url': url, 'file_type': file_type_detected})
        ...
        try:
            ...
            if self.status_callback:
                self.status_callback('download_completed', {'url': url, 'file_type': file_type_detected, 'filename': filename})

            return True
        except Exception as e:
            logging.error(f"Error downloading {url}: {str(e)}")
            if self.status_callback:
                self.status_callback('download_failed', {'url': url, 'reason': str(e)})
            return False
    ...
    def fetch_page_content(self, url: str) -> Optional[str]:
        if self.use_playwright and self.page:
            try:
                ...
                if self.status_callback:
                    self.status_callback('fetch_failed', {'url': url, 'reason': str(e)})
                return None
        else:
            try:
                ...
                if response.status_code == 200:
                    ...
                    if self.status_callback:
                        self.status_callback('fetch_failed', {'url': url, 'reason': f"Status code: {response.status_code}"})
                    return None
            except Exception as e:
    ...
    def extract_urls(self, start_url: str):
        ...
        while queue:
            if self.stop_event.is_set():
                logging.info("Crawl stopped by user.")
                break
            ...
            if self.status_callback:
                self.status_callback('extracting_url', {'url': current_url, 'depth': depth})
            crawled_count += 1
            ...
            if self.is_downloadable_file(current_url):
                ...
                if success:
                    ...
                    if self.status_callback:
                        self.status_callback('file_downloaded', {'url': current_url})
                continue
            ...
            if page_content is None:
                ...
                if self.status_callback:
                    self.status_callback('content_fetch_failed', {'url': current_url})
                continue
            ...
            if self.status_callback:
                self.status_callback('urls_found', {'count': len(child_links)})
    ...
    def extract_content(self, url: str):
        ...
        if page_content is None:
            ...
            if self.status_callback:
                self.status_callback('content_fetch_failed', {'url': url})
            return
        ...
        if not main_content:
            ...
            if self.status_callback:
                self.status_callback('no_main_content', {'url': url})
            return
        ...
        if content:
            ...
            try:
                ...
                if self.status_callback:
                    self.status_callback('content_saved', {'url': url, 'filename': filename})
            except Exception as e:
                logging.error(f"Error saving content for {url}: {str(e)}")
                if self.status_callback:
                    self.status_callback('content_save_failed', {'url': url, 'reason': str(e)})
        else:
            logging.warning(f"No significant content found for: {url}")
            if self.status_callback:
                self.status_callback('no_content', {'url': url})
    ...
    def load_downloaded_files(self):
        ...
        if downloaded_files_path.exists():
            ...
            if self.status_callback:
                self.status_callback('downloaded_files_loaded', {'count': len(self.downloaded_files)})
        else:
            logging.info("No downloaded files tracking file found, starting fresh.")
            if self.status_callback:
                self.status_callback('no_downloaded_files', {})
    ...
    def save_downloaded_files(self):
        ...
        try:
            ...
            if self.status_callback:
                self.status_callback('downloaded_files_saved', {'count': len(self.downloaded_files)})
        except Exception as e:
            logging.error(f"Error saving downloaded files tracking: {str(e)}")
            if self.status_callback:
                self.status_callback('downloaded_files_save_failed', {'reason': str(e)})
    ...
    def generate_report(self, duration: float, error: Optional[str] = None):
        ...
        try:
            ...
            if self.status_callback:
                self.status_callback('report_generated', {'path': str(report_path)})
        except Exception as e:
            logging.error(f"Error generating report: {str(e)}")
            if self.status_callback:
                self.status_callback('report_generation_failed', {'reason': str(e)})
...
"""
        ...
        try:
            ...
            if self.status_callback:
                self.status_callback('summary_generated', {'path': str(self.base_dir / 'summary.txt')})
        except Exception as e:
            logging.error(f"Error generating summary: {str(e)}")
            if self.status_callback:
                self.status_callback('summary_generation_failed', {'reason': str(e)})
    ...
    def generate_json_report(self, duration: float, error: Optional[str] = None):
        ...
        try:
            ...
            if self.status_callback:
                self.status_callback('json_report_generated', {'path': str(json_report_path)})
        except Exception as e:
            logging.error(f"Error generating JSON report: {str(e)}")
            if self.status_callback:
                self.status_callback('json_report_generation_failed', {'reason': str(e)})
    ...
    def generate_xml_sitemap(self):
        ...
        try:
            ...
            if self.status_callback:
                self.status_callback('xml_sitemap_generated', {'path': str(xml_path)})
        except Exception as e:
            logging.error(f"Error generating XML sitemap: {str(e)}")
            if self.status_callback:
                self.status_callback('xml_sitemap_generation_failed', {'reason': str(e)})
    ...
    def save_checkpoint(self):
        ...
        try:
            ...
            if self.status_callback:
                self.status_callback('checkpoint_saved', {'path': CHECKPOINT_FILE})
        except Exception as e:
            logging.error(f"Error saving checkpoint: {str(e)}")
            if self.status_callback:
                self.status_callback('checkpoint_save_failed', {'reason': str(e)})
    ...
    def load_checkpoint(self):
        if os.path.exists(CHECKPOINT_FILE):
            try:
                ...
                if self.status_callback:
                    self.status_callback('checkpoint_loaded', {'path': CHECKPOINT_FILE})
            except Exception as e:
                logging.error(f"Error loading checkpoint: {str(e)}")
                if self.status_callback:
                    self.status_callback('checkpoint_load_failed', {'reason': str(e)})

    def crawl(self):
        start_time = time.time()
        logging.info(f"Starting crawl of {self.start_url}")
        if self.status_callback:
            self.status_callback('crawl_started', {'start_url': self.start_url, 'max_depth': self.max_depth, 'max_urls': self.max_urls})
        ...
        try:
            ...
            if self.status_callback:
                self.status_callback('phase_started', {'phase': 'url_extraction'})
            if not self.visited_pages:
                ...
                if self.status_callback:
                    self.status_callback('checkpoint_found', {'message': 'Skipping URL extraction phase'})

            # Phase 2: Extraction du contenu
            logging.info("Phase 2: Starting content extraction")
            if self.status_callback:
                self.status_callback('phase_started', {'phase': 'content_extraction'})
            ...
            if self.status_callback:
                self.status_callback('phase_completed', {'phase': 'content_extraction'})
            ...
            if self.status_callback:
                self.status_callback('crawl_error', {'error': str(e)})
        ...
        if self.status_callback:
            self.status_callback('crawl_completed', {'duration': duration, 'status': status})

    def stop_crawl(self):
        logging.info("Stop signal received. Stopping crawler...")
        self.stop_event.set()
